...
class CreateEAAlgorithm(QgsProcessingAlgorithm):
    """
    This algorithm creates Enumeration Areas (EAs) by aggregating
    barangay polygons based on building household count and compactness constraints.

    It takes barangay boundaries and building point data (with hhcount field) as inputs,
    and outputs aggregated EA polygons that meet specified household count
    thresholds and spatial constraints.
    """
...
#        )
    ...
    def processAlgorithm(
        ...
        def resolve_ea_parent_barangay(ea_feat):
            if _dc_geo_idx != -1:
                ...
                if val is not None and not (isinstance(val, QVariant) and val.isNull()):
                    ...
                    if val_str.endswith(".0"):
                        val_str = val_str[:-2]
                    if val_str:
                        # Extract 9-digit barangay code if it's a 12-digit EA geocode
                        if len(val_str) > 9:
                            return val_str[:9]
                        return val_str
            ...
            if parent_feat:
                val = parent_feat.attribute(bar_geocode_field)
                if val is not None:
                    val_str = str(val).strip()
                    if val_str.endswith(".0"):
                        val_str = val_str[:-2]
        ...
        if source_crs != building_crs:
            ...
            bbox_transform = QgsCoordinateTransform(source_crs, building_crs, context.transformContext())
...
#            )
        ...
        if self.DELINEATION_CANDIDATE_OUTPUT in parameters and parameters[self.DELINEATION_CANDIDATE_OUTPUT] is not None:
            ...
            for fname in ["split_by", "new_ea", "bldg_count", "bldgpoints_value", "bldgpts_val", "bldgpoint_value"]:
                idx = delin_cand_fields.indexOf(fname)
                if idx != -1:
                    delin_cand_fields.remove(idx)
...
#            feedback.pushInfo(f"Could not set output layer completion name: {str(e)}")
        ...
        for feat in previous_ea_source.getFeatures():
            ...
            try:
                _dc_hh = float(_dc_val) if _dc_val is not None else 0.0
            except (TypeError, ValueError):
                continue
        ...
        feedback.pushInfo(
            f"\nMerge Candidate Index: {len(merge_candidate_ids)} EA(s) flagged "
...
# Stream buildings on-the-fly and match to starting EAs using candidate spatial index memory
        ...
        feedback.pushInfo(f"[DIAGNOSTIC] Total EAs in active barangays (all_ea_features): {len(all_ea_features)}")
        feedback.pushInfo(f"[DIAGNOSTIC] Total delineation candidates identified in Phase 2: {len(delineation_candidate_ids)}")
        feedback.pushInfo(f"[DIAGNOSTIC] Total merge candidates identified in Phase 2: {len(merge_candidate_ids)}")
        feedback.pushInfo(f"[DIAGNOSTIC] Total EAs matching needed criteria: {len(needed_ea_ids)}")
                ...
                for idx in range(len(bar_eas)):
                    if idx in overs:
                        ...
                        if ea.get('from_merge', False):
                            ...
                            fback.pushInfo(f"[Barangay {bar_code}] EA {ea['original_code']} (pop={ea['hh_count']}, buildings={ea.get('bldg_count', 0)}) is a delineation candidate. Calling split_ea...")
                            split_parts = split_ea(ea, max_household, fback)
                            fback.pushInfo(f"[Barangay {bar_code}] EA {ea['original_code']} split_ea returned {len(split_parts)} part(s).")
                            if len(split_parts) > 1:
                                ...
                                if _ea_id in delineation_candidate_hhdivthres:
                                    _parent_hhdivthres = delineation_candidate_hhdivthres[_ea_id]
                                    _max_bldgpv = max(
        ...
        # ── Phase 5: Per-barangay splitting process ─────────────────────────────────────
...
#                feedback.reportError(f"Failed to add EA {i} to sink.")
            ...
            if extracted_buildings_sink is not None:
                ...
                _is_target_ea = (
                    ...
                    or _ea_id in merge_candidate_ids
                    or _ea_id in adjacent_ea_ids
                )
                if _is_target_ea:
...
# ==============================================================================
# Standalone PyQGIS Utility Script
# ==============================================================================

def build_candidates_for_delineation_table(input_layer, output_path=None):
    """
    Builds a Candidates for Delineation table from an input EA polygon layer.

    Requirements:
    - Validate that the 'hhcount' field exists.
    - Handle null or non-numeric hhcount values by skipping those records and logging a warning.
    - Only EAs with hhcount >= 300 are considered Candidates for Delineation.
    - For every qualifying EA, create a record in an output table named Candidates_for_Delineation.
    - The output table contains the ea_number, hhcount, and other identification fields if they exist.
    - Creates output table as a memory layer or saves to a specified file location.
    - Prints total EAs processed and total candidates identified.
    """
    from qgis.core import (
        QgsProject, QgsVectorLayer, QgsField, QgsFeature, QgsFields, QgsVectorFileWriter
    )
    from qgis.PyQt.QtCore import QVariant
    import sys

    # Resolve input layer if passed as string ID/name
    if isinstance(input_layer, str):
        layers = QgsProject.instance().mapLayersByName(input_layer)
        if layers:
            input_layer = layers[0]
        else:
            input_layer = QgsProject.instance().mapLayer(input_layer)

    if not input_layer:
        print("Error: Input layer not found.")
        return None

    # Validate hhcount field exists
    fields = input_layer.fields()
    hhcount_idx = fields.indexOf("hhcount")
    if hhcount_idx == -1:
        # Case-insensitive check fallback
        for i, field in enumerate(fields):
            if field.name().lower() == "hhcount":
                hhcount_idx = i
                break
        if hhcount_idx == -1:
            raise ValueError("Error: The required 'hhcount' field does not exist in the input layer.")

    # Determine unique ID field name (e.g. ea_number, ean, ea_code, id, geocode)
    ea_id_field = None
    for name in ["ea_number", "ean", "ea_code", "id", "geocode"]:
        idx = fields.indexOf(name)
        if idx != -1:
            ea_id_field = fields.at(idx).name()
            break
    if not ea_id_field and fields.count() > 0:
        ea_id_field = fields.at(0).name()

    # Identify identification fields: province, municipality, barangay, etc. if they exist
    id_field_names = []
    if ea_id_field:
        id_field_names.append(ea_id_field)

    for name in ["province", "municipality", "barangay", "prov", "mun", "bgy", "geocode"]:
        for field in fields:
            f_name = field.name()
            if f_name.lower() == name and f_name not in id_field_names:
                id_field_names.append(f_name)

    # Define output table fields
    out_fields = QgsFields()
    out_fields.append(QgsField("ea_number", QVariant.String))
    out_fields.append(QgsField("hhcount", QVariant.Int))

    # Add other identification fields (cast to lower case to standardise)
    for f_name in id_field_names:
        if f_name != ea_id_field:
            orig_field = fields.at(fields.indexOf(f_name))
            out_fields.append(QgsField(f_name.lower(), orig_field.type()))

    # Create memory layer representing Candidates_for_Delineation
    out_layer = QgsVectorLayer("NoGeometry?crs=EPSG:4326", "Candidates_for_Delineation", "memory")
    ...
    for feature in input_layer.getFeatures():
        total_processed += 1
        val = feature.attribute(hhcount_idx)

        if val is None or (isinstance(val, QVariant) and val.isNull()):
            print(f"Warning: Skipping record with null hhcount (Feature ID: {feature.id()}).")
            continue

        try:
            hh_val = float(val)
        except (ValueError, TypeError):
            print(f"Warning: Skipping record with non-numeric hhcount '{val}' (Feature ID: {feature.id()}).")
            continue
        ...
        if hh_val >= 300:
            ...
            ea_num_val = str(feature.attribute(ea_id_field)) if ea_id_field and feature.attribute(ea_id_field) is not None else ""
            new_feat.setAttribute("ea_number", ea_num_val)
            new_feat.setAttribute("hhcount", int(hh_val))

            for f_name in id_field_names:
                if f_name != ea_id_field:
                    orig_val = feature.attribute(f_name)
                    new_feat.setAttribute(f_name.lower(), orig_val)

            out_layer.addFeature(new_feat)

    out_layer.commitChanges()

    print("--------------------------------------------------")
    print(f"Total number of EAs processed: {total_processed}")
    print(f"Total number of delineation candidates identified (hhcount >= 300): {total_candidates}")
    print("--------------------------------------------------")

    if output_path:
        options = QgsVectorFileWriter.SaveVectorOptions()
        options.driverName = "GPKG" if output_path.lower().endswith(".gpkg") else "CSV"
        ...
        if err[0] == QgsVectorFileWriter.WriterError.NoError:
            print(f"Candidates table successfully saved to: {output_path}")
            saved_layer = QgsVectorLayer(output_path, "Candidates_for_Delineation", "ogr")
            QgsProject.instance().addMapLayer(saved_layer)
            return saved_layer
        else:
            print(f"Error saving Candidates table to file: {err[1]}")
            return None
    else:
