...
from .layers_config_widget import LayersConfigWidget
from .mapthemes_config_widget import MapThemesConfigWidget
...
class ProjectConfigurationWidget(WidgetUi, QgsOptionsPageWidget):
    ...
    def _project_config_get(self, name, default=None):
        return getattr(self.__project_configuration, name, default)

    def _project_config_set(self, name, value):
        if hasattr(self.__project_configuration, name):
            setattr(self.__project_configuration, name, value)
    ...
    def reloadProject(self):
        ...
        self.mapThemesConfigWidget = MapThemesConfigWidget(
            self.project, self._project_config_get("map_themes_active_layer", {})
        )
        ...
        self.geofencingShouldPreventDigitizingCheckBox.setChecked(
            self._project_config_get("geofencing_should_prevent_digitizing", False)
        )
        ...
        self.maximumImageWidthHeight.setValue(
            self._project_config_get("maximum_image_width_height", 0)
        )

        self.mapUnitsPerPixel.setValue(self._project_config_get("base_map_mupp", 0))
        self.tileSize.setValue(self._project_config_get("base_map_tile_size", 0))
        self.onlyOfflineCopyFeaturesInAoi.setChecked(
            self._project_config_get("offline_copy_only_aoi", False)
        )
        self.preferOnlineLayersRadioButton.setChecked(
            self._project_config_get("layer_action_preference", "offline") == "online"
        )
        self.preferOfflineLayersRadioButton.setChecked(
            self._project_config_get("layer_action_preference", "offline") == "offline"
        )

        self.forceAutoPush.setChecked(self._project_config_get("force_auto_push", False))
        self.forceAutoPushInterval.setEnabled(
            self._project_config_get("force_auto_push", False)
        )
        self.forceAutoPushInterval.setValue(
            self._project_config_get("force_auto_push_interval_mins", 30)
        )

        attachment_dirs = self.preferences.value("attachmentDirs")
        if not attachment_dirs:
            attachment_dirs = ["DCIM"]
        elif isinstance(attachment_dirs, str):
            attachment_dirs = [attachment_dirs]
        else:
            attachment_dirs = list(attachment_dirs)
    ...
    def apply(self):
        ...
        self._project_config_set(
            "geofencing_should_prevent_digitizing",
        ...
        self._project_config_set("base_map_mupp", float(self.mapUnitsPerPixel.value()))
        self._project_config_set("base_map_tile_size", self.tileSize.value())

        self._project_config_set(
            "maximum_image_width_height",
        ...
        self._project_config_set(
            "layer_action_preference",
            "online" if self.preferOnlineLayersRadioButton.isChecked() else "offline",
        )

        self._project_config_set("force_auto_push", self.forceAutoPush.isChecked())
        self._project_config_set(
            "force_auto_push_interval_mins",
