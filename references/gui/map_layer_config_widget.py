...
from .attachment_naming_widget import AttachmentNamingTableWidget
from .relationship_configuration_widget import (
    RelationshipConfigurationTableWidget,
)
from .utils import set_available_actions
...
class MapLayerConfigWidgetFactory(QgsMapLayerConfigWidgetFactory):
    def __init__(self, title, icon):
        ...
        # Use the C++ setter instead of a Python virtual override.
        # Overriding non-pure-virtual methods in Python causes unstable SIP
        # dispatch in QGIS 3.40 and crashes the layer properties dialog.
        self.setSupportLayerPropertiesDialog(True)

    def createWidget(self, layer, canvas, dock_widget=False, parent=None):
        # createWidget is pure virtual — SIP always dispatches here safely.
        # Do the layer-support check here instead of in supportsLayer so that
        # we never need a Python override of a non-pure-virtual method.
        try:
            import sip
            ...
            if not layer.isValid():
                return None
            # Only unsupported case: ECW rasters
            if layer.source().endswith(".ecw"):
                return None
