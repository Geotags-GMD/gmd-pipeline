...
class LayersConfigWidget(QWidget, LayersConfigWidgetUi):
    ...
    def reloadProject(self):
        ...
        for layer_source in layers:
            ...
            # Auto-assign OFFLINE to _bridge layers that are not yet configured
            if layer_name.endswith('_bridge') and self.get_layer_action(layer_source) != SyncAction.OFFLINE:
                self.set_layer_action(layer_source, SyncAction.OFFLINE)
                layer_source.apply()
